...
const [fetchedData, setFetchedData] = useState({/*title: [], id: []*/ fetchedData: []})
    ...
    const myFetch = async () => {
      ...
      try {
        ...
        setFetchedData({
          ...
          fetchedData: responseParsed.fetchedData
        })
  ...
  const { error, elementsList, inputValue} = elementsListState;
    return (
      <LoginAppStyled>
        ...
        <div>
        <ul>
        {/* {console.log(`${fetchedData.title} title`  )} */}
        {/* {fetchedData.title.map((el, id) => <li value={id} >{el} </li>)} */}
        </ul>
        </div>
      </LoginAppStyled>    
    );
