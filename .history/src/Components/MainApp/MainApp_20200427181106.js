  ...
  useEffect(() => {
    const myFetch = async () => {
      ...
      try {
        ...
        setIsLoading(false)
        setFetchedData({
          title: Object.keys(responseParsed).map(element => element.title),
          id: Object.keys(responseParsed).map(element => element.id)
        })
  ...
  const { error, elementsList, inputValue} = elementsListState;
    return (
      <LoginAppStyled>
        ...
        {console.log(`${element.title} to jest element title data`)}
