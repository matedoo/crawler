...
function MainApp(props) {
  ...
  const [fetchedData, setFetchedData] = useState({title: '', id: 0})
  ...
  useEffect(() => {
    const myFetch = async () => {
      ...
      try {
        ...
        setIsLoading(false)
        setFetchedData({
          title: responseParsed.map(element => element.title),
          id: responseParsed.map(element => element.id)
        })
  ...
  const { error, elementsList, inputValue} = elementsListState;
    return (
      <LoginAppStyled>
        ...
        <ul>
        {/* {fetchedData.title} */}
        {console.log(`${fetchedData} to jest fetched data`)}
        {console.log(`${fetchedData.id} to jest fetched id`)}
        {console.log(`${fetchedData.title} to jest fetched title`)}
        <li value={fetchedData.id} >{fetchedData.title} </li>
        </ul>
        </div>
      </LoginAppStyled>    
    );
