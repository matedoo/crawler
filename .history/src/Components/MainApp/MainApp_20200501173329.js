...
function MainApp(props) {
  ...
  const handleElementEdit = (elName) => {
    console.log(`to jest elid ${elName}`)
    fetch(`http://localhost:4000/items/${elName}`, {
        method: 'PUT', // or 'PUT'
      })
      .then(() => {
        setFetchedData(fetchedData.filter(el => el.id !== elName))
      ...
      <LoginAppStyled>
        ...
        {fetchedData.map((el, index) => (
          <MainAppElement
            ...
            handleEditClick={handleElementEdit}
          />
        ))}
        <input
          name='listelemt'
