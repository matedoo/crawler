...
import ListElement from './MainAppStyled'
...
class MainAppInfo extends React.Component{
    ...
    handleDelete = () =>{
      ...
      console.log(this.state.items);
       ...
       fetch(`http://localhost:4000/items/${this.props.value}`,{
        method: 'DELETE',          
       })
    ...
    render(){
        ...
        return(
           <div>
             ...
             {items.map(item => <ListElement handleDeleteClick={this.handleDelete} item={item.title} value={item.id} />)}
...
class MainApp extends React.Component {
  ...
  render(){
    return (
    <LoginAppStyled>
      <MainAppInfo items={this.myList}></MainAppInfo>
    </LoginAppStyled>    
    );
  }
}
