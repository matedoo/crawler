...
function MainApp(props) {
  ...
  const [elementsListState, setElementsListState] = useState({ elementsList: [], inputValue: ''})
  ...
  const handleButtonClick = () => {
    ...
    if(elementsListState.some(element => element === inputValue)){
      return
    }
    setElementsListState({
  ...
  const { elementsList, inputValue} = elementsListState;
    return (
      <LoginAppStyled>
